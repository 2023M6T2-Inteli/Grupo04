...
def update_analyze(id: int, routeId: int, name: str, startDate: str, endDate: str, supervisor: str, operator: str,
                   createdAt: str) -> tuple[dict[str, str], int]:
    try:
        analyze = Analyze(id=id, routeId=routeId, name=name, startDate=startDate, endDate=endDate,
                          supervisor=supervisor, operator=operator, createdAt=createdAt)
