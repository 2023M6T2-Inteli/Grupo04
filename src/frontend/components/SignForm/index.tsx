...
const SignForm: React.FC<Props> = ({signType}) => {
    ...
    const {
        register,
        handleSubmit
    } = useForm<IFormInputSignUp | IFormInputSignIn>();
    ...
    return (
        <form onSubmit={signType === SignType.Login ? handleSubmit(submitSignIn) : handleSubmit(submitSignUp)}>
            ...
            <label>
                <p className="border-b-2 border-blue-700 w-full rounded font-inter font-xs mb-4">
                    <input className="w-full outline-none ring-0"
            ...
            {signType === SignType.Register && (<label>
                <p className="border-b-2 border-blue-700 w-full rounded font-inter font-xs mb-4">
                    <input
                        className="w-full outline-none ring-0"
                        type="password"
            ...
            <Button type={ButtonType.Home}
                    text={signType === SignType.Register ? "Sign Up" : "Sign" +
                        " In"}
            />
        </form>
    )
